...
|---|---|---|---|---|
| **NOW** | G1 — date picker em UTC puro (`atStartOfDay(ZoneOffset.UTC)` / `atZone(ZoneOffset.UTC).toLocalDate()`) devolve o dia errado para usuários em fusos negativos após 21h locais | DOGFOOD-SIMULATION G1 | P1 | Corretude temporal; fix com fuso local (detalhes no ciclo C) |
| NEXT | G9 — `accessibilityInfoOf` lê apenas `isTouchExplorationEnabled`; `BotanicalMotif` salta para usuários TalkBack mesmo sem reduzir animação, e não lê o setting de movimento reduzido | DOGFOOD-SIMULATION G9 | P2 | Combinar checks; ler `ACCESSIBILITY_ANIMATIONS` |
| NEXT | G2 — copy interpretativa residual "Suas palavras voltaram. Reescreva com o olhar de hoje..." (linha 754) e botão "Reescrever esta nota agora?" (linha 733) | DOGFOOD-SIMULATION G2 | P3 | Neutralizar para factual (identidade sem interpretação) |
| NEXT | G3 — race entre o clear pós-save do draft e o `debounce(700)` pendente: digitação durante a janela de save pode ser perdida | DOGFOOD-SIMULATION G3 | P2 | Mitigação cancel-safe no FioViewModel |
...
- Criado na Missão 3 com G1–G9 do dogfood simulation. G1 entra NOW imediatamente por ser P1 de corretude.
