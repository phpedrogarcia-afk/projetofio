...
| 4 | "MANUS-EXECUTION-QUEUE.md: NOW = integrar `returnPolicy` ao schema 4" | plans/ | A integração retornou como UI-only sobre o engine existente (ADR-043 já implementado: `ReturnPolicy` mapeado a `ELIGIBLE`/`InPeriod`/`OnDate`/`Never` e persistido via campo `originalTimeZone`+state — **a proposta de campo `returnPolicy` em Entry/schema 4 não existe mais no código**); a fila continua pendente de um trabalho que mudou de forma | Alta (NOW falso) |
